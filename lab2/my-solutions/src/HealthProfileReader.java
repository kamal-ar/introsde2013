...
public class HealthProfileReader {
	...
	public static void main(String[] args) {
		...
		// add the case where there are 3 parameters, the third being a string that matches "weight", "height" or "bmi"
			...
			if (healthParam.equals("Height")){
				System.out.println(p.gethProfile().getHeight());
			}
			else if (healthParam.equals("Weight")){
				System.out.println(p.gethProfile().getWeight());
			}
			else if (healthParam.equals("BMI")){
				System.out.println(p.gethProfile().getBMI());
			}
			else {
				System.out.println("Invalid information requested from the healthprofile..");
			}
		}
	}
